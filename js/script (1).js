 ...
 * Gestione navigazione, menu, prenotazioni e Dashboard a 2 colonne con messaggi WhatsApp in grassetto e selettore data.
 */
...
// Inizializza Dashboard Amministratore a 2 Colonne con Selettore Data e Messaggi WhatsApp Formattati in Grassetto
async function initAdminDashboard() {
  ...
  function renderBoard(bookings) {
    // 1. COLONNA DESTRA: Richieste in arrivo (ordinate cronologicamente per arrivo)
    ...
    if (pending.length === 0) {
      ...
      pendingContainer.innerHTML = pending.map(b => {
        ...
        // Messaggio di conferma ufficiale con grassetti richiesti
        ...
        // Messaggio di rifiuto con grassetti
    ...
    // 2. COLONNA SINISTRA: Tavoli confermati (filtrati per la data selezionata)
    ...
    if (displayedConfirmed.length === 0) {
      ...
      confirmedContainer.innerHTML = displayedConfirmed.map(b => {
        const waNum = formatWhatsAppNumber(b.phone);
